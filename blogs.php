...
// Build the SQL query
$sql = "SELECT b.id, b.title, b.subtitle, b.author_name, b.created_at, b.visit, b.is_deleted, bc.name as category_name 
        FROM blogs b 
...
if (isset($_GET['search']) && $_GET['search'] != '') {
    ...
    $whereClause[] = "(b.title LIKE ? OR b.subtitle LIKE ? OR b.author_name LIKE ?)";
...
if (isset($_GET['created']) && $_GET['created'] != '') {
    ...
    switch ($_GET['created']) {
        ...
        case 'this_week':
            ...
            $whereClause[] = "DATE(b.created_at) BETWEEN ? AND ?";
            $params[] = $weekStart;
            $params[] = $weekEnd;
            $types .= "ss";
            break;
...
$categorySql = "SELECT id, name FROM blog_categories";
    ...
    <main class="dashboard-main">
        ...
        <div class="dashboard-main-body">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
                <h6 class="fw-semibold mb-0">Blogs Database</h6>
                <ul class="d-flex align-items-center gap-2">
                    ...
                    <li>-</li>
                    <li class="fw-medium">MOS | Blogs</li>
                </ul>
            </div>
                            ...
                            <tbody>
                                <?php foreach ($blogs as $index => $blog): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="text-md mb-0 fw-normal text-secondary-light"><?php echo htmlspecialchars($blog['title']); ?></span>
                                        </td>
                                        <td><?php echo htmlspecialchars($blog['author_name']); ?></td>
                                        <td><?php echo htmlspecialchars($blog['category_name']); ?></td>
                                        <td><?php echo $blog['visit']; ?></td>
                                        <td class="text-center">
                                            <?php if ($blog['is_deleted'] == 0): ?>
                                                <span class="bg-success-focus text-success-600 border border-success-main px-24 py-4 radius-4 fw-medium text-sm">Active</span>
                                            <?php else: ?>
                                                <span class="bg-danger-focus text-danger-600 border border-danger-main px-24 py-4 radius-4 fw-medium text-sm">Deleted</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="text-center"> 
                                            <div class="d-flex align-items-center gap-10 justify-content-center">
                                                <a href="https://magicofskills.com/blog-details.php?id=<?php echo htmlspecialchars($blog['id']); ?>" type="button" class="bg-info-focus bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle"> 
                                                    <iconify-icon icon="majesticons:eye-line" class="icon text-xl"></iconify-icon>
                                                </a>
                                                <a href="edit-blog.php?id=<?php echo htmlspecialchars($blog['id']); ?>" type="button" class="bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle"> 
                                                    <iconify-icon icon="lucide:edit" class="menu-icon"></iconify-icon>
                                                </a>
                                                <button type="button" class="delete-blog bg-danger-focus bg-hover-danger-200 text-danger-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle" data-blogid="<?php echo $blog['id']; ?>"> 
