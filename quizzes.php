...
if (isset($_GET['date']) && $_GET['date'] != '') {
    ...
    switch ($_GET['date']) {
        case 'today':
            $whereClause[] = "DATE(creation_date) = ?";
            $params[] = $today;
            $types .= "s";
            break;
        case 'upcoming':
        ...
        case 'custom':
            if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
                ...
                $whereClause[] = "DATE(creation_date) BETWEEN ? AND ?";
                $params[] = $startDate;
                $params[] = $endDate;
                $types .= "ss";
            }
...
<head>
    ...
    <title>Quizzes - Magic Of Skills Dashboard</title>
    ...
    <main class="dashboard-main">
        ...
        <div class="dashboard-main-body">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
                <h6 class="fw-semibold mb-0">Quizzes Database</h6>
                <ul class="d-flex align-items-center gap-2">
                    ...
                    <li>-</li>
                    <li class="fw-medium">MOS | Quizzes</li>
                </ul>
            </div>
                        ...
                        <table class="table bordered-table sm-table mb-0">
                            ...
                            <tbody>
                                <?php foreach ($quizzes as $index => $quiz): 
                                    ...
                                    $questionCountSql = "SELECT COUNT(*) FROM questions WHERE quiz_id = ?";
                                    $questionCountStmt = $connect->prepare($questionCountSql);
                                    $questionCountStmt->bind_param("i", $quiz['quiz_id']);
                                    $questionCountStmt->execute();
                                    $questionCount = $questionCountStmt->get_result()->fetch_row()[0];
                                ?>
                                    <tr>
                                        <td><?php echo $offset + $index + 1; ?></td>
                                        <td><?php echo date('d M Y', strtotime($quiz['creation_date'])); ?></td>
                                        <td>
                                            <span class="text-md mb-0 fw-normal text-secondary-light"><?php echo htmlspecialchars($quiz['quiz_name']); ?></span>
                                        </td>
                                        <td><?php echo $quiz['duration_minutes']; ?></td>
                                        <td><?php echo $questionCount; ?></td>
                                        <td class="text-center">
                                            <?php if (strtotime($quiz['creation_date']) > time()): ?>
                                        ...
                                        <td class="text-center"> 
                                            <div class="d-flex align-items-center gap-10 justify-content-center">
                                                <a href="view-quiz.php?id=<?php echo htmlspecialchars($quiz['quiz_id']); ?>" type="button" class="bg-info-focus bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle"> 
                                                    <iconify-icon icon="majesticons:eye-line" class="icon text-xl"></iconify-icon>
                                                </a>
                                                <a href="edit-quiz.php?id=<?php echo htmlspecialchars($quiz['quiz_id']); ?>" type="button" class="bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle"> 
                                                    <iconify-icon icon="lucide:edit" class="menu-icon"></iconify-icon>
                                                </a>
                                                <button type="button" class="delete-quiz bg-danger-focus bg-hover-danger-200 text-danger-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle" data-quizid="<?php echo $quiz['quiz_id']; ?>"> 
...
<script>
    ...
    document.getElementById('downloadExcel').addEventListener('click', function() {
        ...
        var wb = XLSX.utils.table_to_book(table, {sheet: "Quizzes"});
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function(jqXHR, textStatus, errorThrown) {
                            ...
                            Swal.fire(
                                'Error!',
                                'An error occurred while deleting the quiz. Please check the console for details.',
                                'error'
                            );
                        }
                    });
